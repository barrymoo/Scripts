...
parser = argparse.ArgumentParser(usage='./sgen.py infile [options]',description='SLURM Script Generator', prefix_chars='-')
parser.add_argument('infile', help='QM Job File-Supports NWChem, Gaussian, QChem, and ADF')
parser.add_argument('-np', metavar='N', type=int, nargs=1, default=[1], help='Number of processors')
parser.add_argument('-t', metavar='N', type=int, nargs=1, default=[2000], help='Job Time(hours)')
parser.add_argument('-add', metavar='F', type=str, nargs='*', default=[0], help='Additional Job Files')
parser.add_argument('-g', action='store_true', help='Submit to general compute nodes')
...
if fileExt != 'nw' and fileExt != 'adf' and fileExt != 'com' and fileExt != 'qc':
  print ('This program expects an input file with the extension: nw, adf, com, qc')
  sys.exit()
...
if fileExt == 'nw':
  ...
  ofile.write('HOSTNAME=`srun hostname -s`\n')
  ...
  ofile.write('ulimit -c 0\n')
  ofile.write('ulimit -a\n')
  ofile.write('sbcast $SLURM_SUBMIT_DIR/'+inpfile+' $SLURMTMPDIR/'+inpfile+'\n')
  if addfilelist[0] != 0:
    for i in range(0,len(addfilelist)):
      ofile.write('sbcast $SLURM_SUBMIT_DIR/'+addfilelist[i]+' $SLURMTMPDIR/'+addfilelist[i]+'\n')
  ofile.write('mpirun -np $NPROCS $NWCHEM_EXECUTABLE '+inpfile+'\n')
  ...
  if addfilelist[0] != 0:
    for i in range(0,len(addfilelist)):
      ...
      if tape == '.t21' or dummy == 'TAPE21':
        print ('Found TAPE21!')
        ofile.write('sbcast $SLURM_SUBMIT_DIR/'+addfilelist[i]+' $SLURMTMPDIR/TAPE21\n')
      else:
        ofile.write('sbcast $SLURM_SUBMIT_DIR/'+addfilelist[i]+' $SLURMTMPDIR/'+addfilelist[i]+'\n')
  ...
  ofile.write('export LC_COLLATE=C\n')
  ofile.write('source '+tagfile+'\n')
  ofile.write('export LD_LIBRARY_PATH=/util/pgi/linux86/6.1/lib:$LD_LIBRARY_PATH\n')
  ofile.write('sbcast $SLURM_SUBMIT_DIR/'+inpfile+' $SLURMTMPDIR/'+inpfile+'\n')
  ...
  ofile.write('srun --nodes=${SLURM_NNODES} bash -c \'hostname\' > $PBS_NODEFILE\n')
