...
try:
    ...
    if arguments['--civecs'] and (arguments['--beta'] or arguments['--alpha-beta']):
        exit('Input Error: -b/--beta and -a/--alpha-beta options cannot be used with -c/--civecs option')
        ...
        if arguments['--alpha-beta']:
            for spin in ['alpha', 'beta']:
                for i in mos:
                    ...
                    f.write('spin {0}\n'.format(spin))
                    f.write('orbitals view;1;{0}\n'.format(i))
                    f.write('output {0}-{1}-{2}.cube\n'.format(base, spin, i))
                    end_dplot_block(f)
        elif arguments['--beta']:
            spin = 'beta'
            for i in mos:
